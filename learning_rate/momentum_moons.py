...
learning_rate = [0.01, 0.5, 1] # learning rate for gradient descent
...
def neural_network(nn_hidden_units, alpha, num_epochs=2000):
  ...
  print "learning_rate: %s, friction: %s" % (alpha[0], alpha[1])
  learning_rate = alpha[0]
  friction = alpha[1]
  dW2_velocity = alpha[2]
  dW1_velocity = alpha[2]
  ...
  for i in xrange(0, num_epochs):
    ...
    # Update learning rate to include momentum
    dW2_velocity = (friction * dW2_velocity) - (learning_rate * dW2)
    dW1_velocity = (friction * dW1_velocity) - (learning_rate * dW1)
